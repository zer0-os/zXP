...
contract SeasonRegistry is IRegistry, Owned, Utils {
    enum ObjectType {
        MANAGER,
        CHAR,
        ITEM,
    ...
    function registerAddress(
        ...
        objects[_contractName].contractAddress[
            currentSeason
        ] = _contractAddress;
    ...
    ) public ownerOnly validAddress(_newContractAddress) {
        // validate input
        require(_contractName.length > 0, "ERR_INVALID_NAME");
        // validate contract name is registered
        require(
            objects[_contractName].seasonZero != 0,
            "ERR_UNREGISTERED_NAME"
        );
