...
describe("ZXP", () => {
    ...
    before(async () => {
        ...
        const erc20Contracts = await ethers.getContractFactory("MockERC20");
        const erc20 = await erc20Contracts.deploy("zToken", "WILD");
        await erc20.waitForDeployment();
        mockErc20 = erc20;

        const erc721Contracts = await ethers.getContractFactory("MockERC721");
        const erc721 = await erc721Contracts.deploy("zToken", "WILD", "");
        await erc721.waitForDeployment();
        mockErc721 = erc721;

        const gameRegFactory = await ethers.getContractFactory("GameRegistry");
        const gameRegDeploy = await gameRegFactory.deploy();
        await gameRegDeploy.waitForDeployment();
        gameRegistry = gameRegDeploy;

        gameName = ethers.utils.formatBytes32String("game");
        const seasonRegFactory = await ethers.getContractFactory("SeasonRegistry");
        ...
        const gameVaultFactory = await ethers.getContractFactory("GameVault");
        ...
        const top3rewardsFactory = await ethers.getContractFactory("PlayerRewards");
        ...
        const stakerRewardsFactory = await ethers.getContractFactory("StakerRewards");
    ...
    for (let index = 0; index < numSeasons; index++) {
        ...
        it("Mints Staker 2 NFT", async () => {
            //s2nft = ethers.keccak256(ethers.utils.formatBytes32String((index + 1).toString()));
            s2nft = s2nft + 3;
            console.log(s2nft);
